...
//        echo  $this->db->last_query();die;
    ...
    public  function getAllTripByCustomerApi($userId,$roleId,$status) {
        $this->db->select(array("*"))
                ->from("tbl_trip")
                ->join('vehicle', 'vehicle.v_Id=tbl_trip.t_vehicle_id')
                ->join('users', 'users.Id=tbl_trip.t_user_Id')
                ->where(array("tbl_trip.t_user_Id" => $userId ,"tbl_trip.t_status" => $status ,"users.Status" => 1, "users.Role_Id" => $roleId,"users.deletion_status" => 0));
        $query = $this->db->get();
       // echo  $this->db->last_query();die;
         if($query->num_rows() > 0){
                $data= $query->result();
